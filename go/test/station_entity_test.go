...
func TestStationEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Station(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Station(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set PUBLIBIKE_STATIONS_TEST_STATION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		stationRef01DataDt0LoadResult := core.ToMapAny(entityData(stationRef01DataDt0Loaded))
...
func stationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("PUBLIBIKE_STATIONS_TEST_STATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PUBLIBIKE_STATIONS_TEST_STATION_ENTID"])
