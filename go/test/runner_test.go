...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("PUBLIBIKESTATIONS_TEST_LIVE") == "TRUE" ||
		os.Getenv("PUBLIBIKESTATIONS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("PUBLIBIKESTATIONS_TEST_EXPLAIN"); explain != "" {
