# PublibikeStations Golang SDK



The Golang SDK for the PublibikeStations API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
func main() {
    ...
    // List station records — the value is the array of records itself.
    stations, err := client.Station(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range stations.([]any) {
        fmt.Println(item)
    }

    // Load a single station — the value is the loaded record.
...
station, err := client.Station(nil).List(
    nil, nil,
)
if err != nil {
    panic(err)
}
fmt.Println(station) // the returned mock data
```
...
### NewPublibikeStationsSDK

```go
func NewPublibikeStationsSDK(options map[string]any) *PublibikeStationsSDK
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *PublibikeStationsSDK
```
...
| `Station` | `(data map[string]any) PublibikeStationsEntity` | Create a Station entity instance. |

### Entity interface (PublibikeStationsEntity)

All entities implement the `PublibikeStationsEntity` interface.
...
slice):
    ...
    station, err := client.Station(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
    // station is the returned record
...
fmt.Println(stations) // the array of records
```
...
```
github.com/voxgig-sdk/publibike-stations-sdk/go/
├── publibike-stations.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/publibike-stations-sdk/go`) re-exports everything needed
