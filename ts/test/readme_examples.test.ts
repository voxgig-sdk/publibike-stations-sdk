...
//     (\`new PublibikeStationsSDK(...)\` / \`PublibikeStationsSDK.test(...)\` become
//     \`PublibikeStationsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'PublibikeStationsSDK'
...
const TEST_SEED = {"entity":{"station":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PublibikeStationsSDK(...)\` and any
// \`PublibikeStationsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PublibikeStationsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PublibikeStationsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
