...
class StationEntity
{
    ...
    {
        if ($args) {
            $this->_data = PublibikeStationsHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = PublibikeStationsHelpers::to_map(Struct::clone($args)) ?? [];
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = PublibikeStationsHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
