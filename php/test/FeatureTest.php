...
// PublibikeStations SDK feature test
//
...
class FtCtrl extends PublibikeStationsControl
{
...
class FtClient
{
    ...
    private PublibikeStationsUtility $utility;

    public function __construct(PublibikeStationsUtility $utility, array $options)
    {
    ...
    public function get_utility(): PublibikeStationsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PublibikeStationsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PublibikeStationsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PublibikeStationsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PublibikeStationsConfig::make_feature($name);
    ...
    private static function buildUrl(PublibikeStationsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PublibikeStationsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PublibikeStationsResponse($fetched);
        }
    ...
    private function fail(PublibikeStationsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PublibikeStationsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
