...
The PHP SDK for the PublibikeStations API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Station(null)->list(null, null);
...
```php
[$result, $err] = $client->Station(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->PublibikeStations(null)->load(
    ["id" => "test01"], null
);
