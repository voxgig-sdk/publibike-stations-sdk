# PublibikeStations PHP SDK



The PHP SDK for the PublibikeStations API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Station()` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
| `network` | `array` | Representation of a network |
| `sponsors` | `array` | An array of sponsors of this station |
| `state` | `array` | Representation of a state. |
...
```
php/
├── publibikestations_sdk.php          -- Main SDK class
...
The main class (`publibikestations_sdk.php`) exports the SDK class
