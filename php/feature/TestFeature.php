...
// PublibikeStations SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class PublibikeStationsTestFeature extends PublibikeStationsBaseFeature
{
    ...
    public function init(PublibikeStationsContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (PublibikeStationsContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = PublibikeStationsParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (PublibikeStationsContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(PublibikeStationsContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = PublibikeStationsParam::call($ctx, $k);
