...
from publibikestations_sdk.core.helpers import get_ctx_prop, to_map
...
class PublibikeStationsContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = PublibikeStationsControl()
        ...
        if isinstance(sp, PublibikeStationsSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, PublibikeStationsResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, PublibikeStationsResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return PublibikeStationsOperation({})
    ...
    def make_error(self, code, msg):
        return PublibikeStationsError(code, msg, self)
