...
class TestStationDirect:
    ...
    def test_should_direct_list_station(self):
        setup = _station_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-station", "live" if setup["live"] else "unit")
        if _skip:
