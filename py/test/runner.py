# PublibikeStations SDK test runner
...
class PublibikeStationsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PublibikeStationsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PublibikeStationsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PublibikeStationsTestRunner.getenv("PUBLIBIKE_STATIONS_TEST_LIVE")
        override = PublibikeStationsTestRunner.getenv("PUBLIBIKE_STATIONS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PublibikeStationsTestRunner.getenv(key)
        ...
        explain = PublibikeStationsTestRunner.getenv("PUBLIBIKE_STATIONS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["PUBLIBIKE_STATIONS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if PublibikeStationsTestRunner._test_control is not None:
            return PublibikeStationsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PublibikeStationsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PublibikeStationsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PublibikeStationsTestRunner.load_test_control()
...
def load_env_local():
    PublibikeStationsTestRunner.load_env_local()


def env_override(m):
    return PublibikeStationsTestRunner.env_override(m)


def entity_data(v):
    return PublibikeStationsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PublibikeStationsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PublibikeStationsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PublibikeStationsTestRunner.load_test_control()


def live_delay_ms():
    return PublibikeStationsTestRunner.live_delay_ms()
