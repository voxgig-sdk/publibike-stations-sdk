...
The Python SDK for the PublibikeStations API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Station(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Station(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.PublibikeStations(None).load(
    {"id": "test01"}, None
)
