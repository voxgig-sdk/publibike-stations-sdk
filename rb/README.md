...
The Ruby SDK for the PublibikeStations API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Station(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.PublibikeStations(nil).load(
  { "id" => "test01" }, nil
)
