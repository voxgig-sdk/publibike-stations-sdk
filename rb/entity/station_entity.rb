# PublibikeStations SDK Station entity
...
class StationEntity
  ...
  def data_set(args)
    if args
      @_data = PublibikeStationsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = PublibikeStationsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Station, Hash] the loaded Station; raises PublibikeStationsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PublibikeStationsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [StationListMatch, Hash, nil] match filter (any subset of
  #   Station fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Station>, Array] the matching Station items; raises PublibikeStationsError on failure
