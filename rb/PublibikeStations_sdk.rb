...
require_relative 'PublibikeStations_types'


class PublibikeStationsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PublibikeStationsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PublibikeStationsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PublibikeStationsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PublibikeStationsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PublibikeStationsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PublibikeStationsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PublibikeStationsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => PublibikeStationsError.new(
        "#{op}_allow",
        "PublibikeStationsSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue PublibikeStationsError => err
    ...
    if fetched.is_a?(Hash)
      status = PublibikeStationsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PublibikeStationsError.new(
        "graphql_error", "PublibikeStationsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = PublibikeStationsSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
