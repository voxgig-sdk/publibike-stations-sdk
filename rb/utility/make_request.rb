...
module PublibikeStationsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PublibikeStationsResponse.new({})
    ...
    if fetch_err
      ...
      response = PublibikeStationsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PublibikeStationsResponse.new(fetched)
    else
