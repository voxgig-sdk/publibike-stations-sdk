...
module PublibikeStationsFeatureHarness
  # True when this SDK was generated with the named feature.
  def self.has_feature?(name)
    f = PublibikeStationsConfig.shared_config["feature"]
  ...
  class Harness
    ...
    def initialize(features, server: nil, base: "http://api.test", headers: {})
      ...
      @utility.fetcher = server || PublibikeStationsFeatureHarness.default_server
      ...
      features.each do |fspec|
        name = fspec["name"]
        next unless PublibikeStationsFeatureHarness.has_feature?(name)
        f = PublibikeStationsFeatures.make_feature(name)
    ...
    def op(opname: "load", entity: "widget", method: nil, path: nil, query: nil,
      ...
      method ||= PublibikeStationsFeatureHarness.default_method(opname)
      ...
      begin
        fire(ctx, "PrePoint")
        raise ctx.out["point"] if ctx.out["point"].is_a?(PublibikeStationsError)
        ...
        ctx.response = fetched.is_a?(Hash) ? PublibikeStationsResponse.new(fetched) : nil
    ...
    def populate_result(ctx, fetched, fetch_err)
      result = PublibikeStationsResult.new({})
      ctx.result = result
...
class FeatureTest < Minitest::Test
  H = PublibikeStationsFeatureHarness
  ...
  def test_retry_retries_a_transport_error_tuple
    ...
    server, calls = H.recording_server { |n, _fd|
      n < 3 ? [nil, PublibikeStationsError.new("boom", "boom")] : [H.make_response(200, { "ok" => true }), nil]
    }
  ...
  def test_retry_exhausted_transport_error_surfaces
    skip_unless_feature("retry")
    clock = H::Clock.new
    server, calls = H.recording_server { |_n, _fd| [nil, PublibikeStationsError.new("boom", "boom")] }
    h = harness([fspec("retry",
      "retries" => 2, "minDelay" => 1, "jitter" => false, "sleep" => clock.sleeper)],
      server: server)
    res = h.op
