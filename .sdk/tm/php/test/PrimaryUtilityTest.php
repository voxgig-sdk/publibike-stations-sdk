...
// PublibikeStations SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        PublibikeStationsSDK $client,
        PublibikeStationsUtility $utility
    ): PublibikeStationsContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new PublibikeStationsContext($ctxmap, null);
    ...
    private static function fixctx(PublibikeStationsContext $ctx, PublibikeStationsSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?PublibikeStationsError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new PublibikeStationsError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new PublibikeStationsResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new PublibikeStationsBaseFeature();
    ...
    {
        $hook_client = PublibikeStationsSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends PublibikeStationsBaseFeature {
            public $hook_fn = null;
            public function TestHook(PublibikeStationsContext $ctx): void
            {
    ...
    {
        $init_client = PublibikeStationsSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends PublibikeStationsBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new PublibikeStationsSDK([
            'system' => [
    ...
    {
        $blocked_client = new PublibikeStationsSDK([
            'system' => [
    ...
    public function test_make_fetch_def_basic(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new PublibikeStationsSpec([
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new PublibikeStationsSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = PublibikeStationsSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new PublibikeStationsOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = PublibikeStationsSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof PublibikeStationsError) ? $result->err->msg : (string)$result->err;
