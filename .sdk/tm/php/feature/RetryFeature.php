...
// PublibikeStations SDK retry feature
...
class PublibikeStationsRetryFeature extends PublibikeStationsBaseFeature
{
    ...
    public function init(PublibikeStationsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (PublibikeStationsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(PublibikeStationsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(PublibikeStationsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
