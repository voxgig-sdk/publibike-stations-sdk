// Command PublibikeStations-cli is a CLI + REPL for the PublibikeStations SDK,
...
const prompt = "publibike-stations"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "station"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PUBLIBIKE_STATIONS_APIKEY (api key), PUBLIBIKE_STATIONS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PUBLIBIKE_STATIONS_APIKEY carries the API key and
	// PUBLIBIKE_STATIONS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PUBLIBIKE_STATIONS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PUBLIBIKE_STATIONS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPublibikeStationsSDK(opts)
