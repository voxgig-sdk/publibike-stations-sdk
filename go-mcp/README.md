...
[MCP](https://modelcontextprotocol.io) server exposing the PublibikeStations SDK as
two agent tools — `publibike-stations_list` and `publibike-stations_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/publibike-stations-mcp)
make build
...
```jsonc
// publibike-stations_list: first page of records
{ "entity": "station" }
{ "entity": "station", "query": { } }

// publibike-stations_load: one record by id
{ "entity": "station", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/publibike-stations-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user publibike-stations \
     -- "$PWD"/dist/*/publibike-stations-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `publibike-stations_list` and `publibike-stations_load` tools now appear
   in new sessions. Ask the agent to *"list station using publibike-stations"*
   and it calls `publibike-stations_list` with `{"entity":"station"}`.
...
### Call the `publibike-stations_list` tool
...
### Call the `publibike-stations_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `publibike-stations_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `publibike-stations_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
