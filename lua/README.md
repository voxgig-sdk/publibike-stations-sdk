# PublibikeStations Lua SDK



The Lua SDK for the PublibikeStations API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Station()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── publibike-stations_sdk.lua    -- Main SDK module
...
The main module (`publibike-stations_sdk`) exports the SDK constructor
